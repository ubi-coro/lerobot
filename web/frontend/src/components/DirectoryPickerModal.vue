  ...
  <div v-if="modelValue" class="dir-modal-overlay" @keydown.esc="close">
    <div class="dir-modal" role="dialog" aria-modal="true">
      ...
      <footer class="dm-footer">
        <div class="left">Selected: <strong>{{ selectedPath || 'None' }}</strong></div>
        <div class="right">
...
.dir-modal { width:720px; max-width:95vw; background:#fff; border-radius:12px; box-shadow:0 10px 30px rgba(0,0,0,0.35); display:flex; flex-direction:column; max-height:80vh; }
...
.dm-body { flex:0 0 340px; overflow:auto; padding:.5rem 1rem .75rem; }
...
.icon { font-size:.9rem; }
.dm-footer { display:flex; align-items:center; justify-content:space-between; padding:.65rem 1rem .75rem; border-top:1px solid #e5e7eb; gap:1rem; flex-wrap:wrap; }
.dm-footer .right { display:flex; gap:.55rem; }
.dm-footer button { background:#3b82f6; color:#fff; border:none; padding:.55rem .9rem; border-radius:6px; font-size:.7rem; font-weight:600; cursor:pointer; }
...
@media (max-width:600px){ .dir-modal { width:100%; height:100%; border-radius:0; max-height:none; } ul.folder-list { grid-template-columns:repeat(auto-fill,minmax(140px,1fr)); } }
